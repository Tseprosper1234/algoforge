...
// services/emailService.js - Using Resend API
...
async function sendEmail(to, subject, html) {
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: process.env.EMAIL_FROM || 'AlgoForge <onboarding@resend.dev>',
      to: [to],
      subject,
      html,
    });
...
async function sendVerificationEmail(to, code) {
  const html = `
    ...
    <body style="font-family: Arial, sans-serif; text-align: center;">
      ...
      <p>Your verification code is:</p>
      <div style="font-size: 32px; font-weight: bold; background: #f0f0f0; display: inline-block; padding: 10px 20px; border-radius: 8px; letter-spacing: 4px;">
        ${code}
      </div>
...
// Notify about new content (optional)
async function notifyNewContent(to, fileName, fileId) {
  const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
  const html = `
    <!DOCTYPE html>
    <html>
    <head><meta charset="UTF-8"><title>New Content Available</title></head>
    <body style="font-family: Arial, sans-serif; text-align: center;">
      <h2 style="color: #3b82f6;">AlgoForge</h2>
      <p>New learning topic added:</p>
      <h3>${fileName}</h3>
      <a href="${frontendUrl}/files/${fileId}" style="display: inline-block; background: #3b82f6; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">View Now</a>
    </body>
    </html>
  `;
  return await sendEmail(to, 'New Content Available - AlgoForge', html);
